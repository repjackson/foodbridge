if Meteor.isClient
    ...
    Router.route '/source/:doc_id/view', (->
        @layout 'layout'
        @render 'source_view'
        ), name:'source_view_long'
    
    
    Template.sources.onCreated ->
        @autorun => @subscribe 'source_docs',
            picked_tags.array()
            Session.get('source_title_filter')

        @autorun => @subscribe 'source_facets',
            picked_tags.array()
            Session.get('source_title_filter')
    ...
    Template.user_sources.onCreated ->
        @autorun => Meteor.subscribe 'user_sources', Router.current().params.username, ->
    ...
    Template.source_edit.onCreated ->
        @autorun => Meteor.subscribe 'doc_by_id', Router.current().params.doc_id, ->
    Template.source_card.onCreated ->
        @autorun => Meteor.subscribe 'doc_comments', @data._id, ->


    Template.source_card.events
        'click .view_source': ->
    ...
    Template.source_edit.events
        ...
        'click .publish': ->
            Swal.fire({
                ...
                if result.value
                    Meteor.call 'publish_source', @_id, =>
                        Swal.fire(
                            position: 'bottom-end',
                            icon: 'success',
        ...
        'click .unpublish': ->
            Swal.fire({
                ...
                if result.value
                    Meteor.call 'unpublish_source', @_id, =>
                        Swal.fire(
                            position: 'bottom-end',
                            icon: 'success',
...
if Meteor.isServer
    Meteor.publish 'user_sources', (username)->
    ...
    Meteor.publish 'source_count', (
        ...
        if source_query and source_query.length > 1
            console.log 'searching source_query', source_query
            match.title = {$regex:"#{source_query}", $options: 'i'}
        Counts.publish this, 'source_counter', Docs.find(match)
        return undefined
